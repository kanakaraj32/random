...
import TabItem from './components/Image'
...
class App extends Component {
  state = {
    textContent: tabsList[0].tabId,
  }

  filteredData = () => {
    const {textContent} = this.state
    const filtered = imagesList.filter(each => each.category === textContent)
    return filtered
  }

  tabclick = tabId => {
    this.setState({textContent: tabId})
  }

  img = () => {
    const random = Math.floor(Math.random() * imagesList.length)
    const imga = imagesList[random]
    return imga
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div className="raju">
          <img src={imageFilter} alt="ima" className="image" />
          <ul className="tabs-container">
            {tabsList.map(details => (
              <Text
                key={details.tabId}
                detail={details}
